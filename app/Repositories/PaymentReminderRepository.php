...
class PaymentReminderRepository extends BaseRepository
{
	 ...
	 * @return  \App\Models\PaymentReminder
	 */
	...
	public function addReminderable($reminderable)
	{
		try {
			...
			$this->setSuccess('Successfully add reminderable to payment reminder.');
		} catch (QueryException $qe) {
			$error = $qe->getMessage();
			$this->setError('Failed to add reminderable to payment reminder', $error);
		}
	...
	public function removeReminderable($reminderable)
	{
		try {
			...
			if (! $type !== PaymentReminderable::class) {
				$paymentReminderable = PaymentReminderable::wherePaymentReminder($reminder)
	...
	public function removeMultipleReminderable(array $reminderables)
	{
		...
		try {
			...
			$ids = array_map(function ($reminderable) {
				if (isset($reminderable['id'])) return $reminderable['id'];
			}, $reminderables);
			PaymentReminderable::whereIn('payment_reminderable_id', $ids)->destroy();
