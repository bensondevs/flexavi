...
class DeleteWorkServiceRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        return $this->user()
            ->fresh()
            ->can('delete-work-service', $workService);
    }
    ...
    public function getWorkService(): ?WorkService
    {
        ...
        $id = $this->input('id') ?: $this->input('work_service_id');

        return $this->workService = WorkService::withTrashed()->findOrFail($id);
    }
    ...
    public function rules(): array
    {
        ...
        return [
            'work_service_id' => [
                'required',
                'string',
                Rule::exists('work_services', 'id')->where('company_id', $company->id)
            ],
        ];
    }
}
