...
class ValidateTokenRequest extends FormRequest
{
    ...
    public function getResetPasswordToken(): ?PasswordReset
    {
        ...
        $resetPasswordToken = PasswordReset::where('email', $this->input('email'))->where('token', $this->input('token'))->first();

        if (!$resetPasswordToken) {
            return abort(422, 'Failed to get reset password token, token invalid!');
        }
