<?php

namespace App\Http\Requests\Company\Settings;

use App\Models\Setting\EmployeeSetting;
use App\Traits\CompanyPopulateRequestOptions;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Support\Arr;

class UpdateEmployeeSettingRequest extends FormRequest
{
    use CompanyPopulateRequestOptions;

    /**
     * Setting instance container
     *
     * @return ?EmployeeSetting
     */
    private $setting;

    /**
     * Get setting model
     *
     * @return EmployeeSetting
     */
    public function getSetting(): EmployeeSetting
    {
        if ($this->setting) {
            return $this->setting;
        }
        $setting = EmployeeSetting::query()
            ->firstOrNew(['company_id' => $this->getCompany()->id]);

        if ($setting->wasRecentlyCreated) {
            $setting->fill(
                Arr::except(
                    EmployeeSetting::query()->default()->attributesToArray(),
                    ['id','company_id']
                )
            );
            $setting->save();
        }

        return $this->setting = $setting;
    }

    /**
     * Determine if the user is authorized to make this request.
     *
     * @return bool
     */
    public function authorize()
    {
        return $this->user()
            ->fresh()
            ->can('edit-setting');
    }

     /**
     * Prepare input for validation.
     *
     * This will prepare input to configure the loadable relationships
     *
     * @return void
     */
    protected function prepareForValidation()
    {
        if ($value = $this->get('use_initials_when_dont_have_avatar')) {
            $this->merge(['use_initials_when_dont_have_avatar' => strtobool($value)]);
        }
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array
     */
    public function rules()
    {
        return [
            'pagination' => 'nullable|numeric',
            'use_initials_when_dont_have_avatar' => 'nullable|boolean',
            'invitation_expiry' => 'nullable|numeric',
        ];
    }
}
