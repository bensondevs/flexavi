...
/**
 * @see \App\Services\Subscription\Notification\SubscriptionPaymentNotificationService::paymentExpired()
 *      To the tested service method.
 */
class PaymentExpiredTest extends SubscriptionPaymentNotificationServiceTest
{
    /**
    ...
    public function ensure_this_method_pushed_job(): void
    {
        ...
        $this->service()->paymentExpired($subscriptionPayment);
        \Queue::assertPushed(SendMail::class, function ($job) {
            return $job->mailable instanceof SubscriptionPaymentExpired;
        });
    }
}
